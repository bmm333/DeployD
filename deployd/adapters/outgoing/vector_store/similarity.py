...
def blend_rrf(
    ...
    structural_hits_component = sorted(
        structural_hits, key=lambda x: x.component_score, reverse=True
    )

    # Process Semantic (Dense)
    for rank, dense_hit in enumerate(dense_hits, start=1):
        if dense_hit.semantic_score > 0:
            scores[dense_hit.runbook_id] = replace(
                scores.get(dense_hit.runbook_id, ScoreSet()), semantic=dense_hit.semantic_score
            )
            rrf_scores[dense_hit.runbook_id] = rrf_scores.get(dense_hit.runbook_id, 0.0) + 1.0 / (
                k + rank
            )
        else:
            scores[dense_hit.runbook_id] = replace(
                scores.get(dense_hit.runbook_id, ScoreSet()), semantic=0.0
            )

    # Process BM25 (Sparse)
    for rank, sparse_hit in enumerate(spares_hits, start=1):
        if sparse_hit.bm25_score > 0:
            scores[sparse_hit.runbook_id] = replace(
                scores.get(sparse_hit.runbook_id, ScoreSet()), bm25=sparse_hit.bm25_score
            )
            rrf_scores[sparse_hit.runbook_id] = rrf_scores.get(sparse_hit.runbook_id, 0.0) + 1.0 / (
                k + rank
            )
        else:
            scores[sparse_hit.runbook_id] = replace(
                scores.get(sparse_hit.runbook_id, ScoreSet()), bm25=0.0
            )

    # Process Causal and Component
    # Causal
    for rank, struct_hit_causal in enumerate(structural_hits_causal, start=1):
        if struct_hit_causal.causal_score > 0:
            scores[struct_hit_causal.runbook_id] = replace(
                scores.get(struct_hit_causal.runbook_id, ScoreSet()),
                causal=struct_hit_causal.causal_score,
            )
            rrf_scores[struct_hit_causal.runbook_id] = rrf_scores.get(
                struct_hit_causal.runbook_id, 0.0
            ) + 1.0 / (k + rank)
        else:
            scores[struct_hit_causal.runbook_id] = replace(
                scores.get(struct_hit_causal.runbook_id, ScoreSet()), causal=0.0
            )

    # Component
    for rank, struct_hit_component in enumerate(structural_hits_component, start=1):
        if struct_hit_component.component_score > 0:
            scores[struct_hit_component.runbook_id] = replace(
                scores.get(struct_hit_component.runbook_id, ScoreSet()),
                component=struct_hit_component.component_score,
            )
            rrf_scores[struct_hit_component.runbook_id] = rrf_scores.get(
                struct_hit_component.runbook_id, 0.0
            ) + 1.0 / (k + rank)
        else:
            scores[struct_hit_component.runbook_id] = replace(
                scores.get(struct_hit_component.runbook_id, ScoreSet()), component=0.0
            )
