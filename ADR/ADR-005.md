...
- **Rules perform their own I/O (Database/Config reading)**: Rejected. This would pollute the domain layer with infrastructure concerns, making rules hard to test and mocking databases mandatory for unit tests.
- **Detection logic fully implemented in the application layer**: Rejected. Moving the logic out of the domain layer would result in an anemic domain model and highly procedural application orchestrators, making the core business logic harder to reason about and evolve.
